...
int sort_int_array(int * intArray, int arrayLen)
{
	...
	if (!intArray)  //Checking for NULL
	{
		return -1;
	}
	else if (arrayLen < 2) // Checking for invalid input
	{
		return -2;
	}
	else
	{
		for (j = 0; j < arrayLen; j++) // Outerloop to test entire string
		{
			for (i = arrayLen - 1; i > j; i--) // Inner loop
			{
