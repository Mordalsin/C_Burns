...
int replace_character(char * string, const char findThisChar, const char replaceItWithThis)
{
	...
	if (findThisChar > 126 || findThisChar < 32)
	{
		return ERR_NON_PRINTABLE_CHARACTER;
	}
	if (replaceItWithThis > 126 || replaceItWithThis < 32)
	{
		return ERR_NON_PRINTABLE_CHARACTER;
	}
	if (string)
	...
	return newLines; // You'll want to modify the return statement for this function
