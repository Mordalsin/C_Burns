...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	...
	int i = 0;
	int j = 0;
	char * anotherPointer_ptr = sentence_ptr;
	char * tempReturn_ptr = NULL;
	char * tempSearch_ptr = NULL;
	...
	else if (strlen(searchWord_ptr) > strlen(sentence_ptr))
	{
	...
	{
		for (i = 0; i < strlen(sentence_ptr); i++)						// Loop through the sentence provided
		{
			if ((*(anotherPointer_ptr)) == (*(searchWord_ptr)))			// Check for the start of the same word
			{
				tempReturn_ptr = anotherPointer_ptr;					// Save the state of anotherPointer
				tempSearch_ptr = searchWord_ptr;
				for (j = 1; j < strlen(searchWord_ptr); j++)			// Loop through the search word
				{
					if ((*(anotherPointer_ptr)) == (*(tempSearch_ptr)))	// Check for the letters
					{
						anotherPointer_ptr++;							// Keep looking through
						tempSearch_ptr++;								// the string 
					}
					else
					{
						break;											// Break out of not the same
					}
				}
				if (j == strlen(searchWord_ptr))						// Check to make sure the word was the same length
				{
					flag = 1;
					(*(errorCode_ptr)) = ERROR_CODE_SUCCESS;			// Return success
					return tempReturn_ptr;
				}
			}
			anotherPointer_ptr++;
		}
	...
	if (flag == 0)
	{
		(*(errorCode_ptr)) = ERROR_SEARCH_NOT_FOUND;					// There were no search words
		return NULL;
	}
}
